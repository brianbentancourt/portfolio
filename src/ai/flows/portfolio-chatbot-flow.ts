...
const PortfolioChatbotInputSchema = z.object({
  ...
  portfolioContext: z.string().describe('Contextual information about Brian Bentancourt, his skills, projects, and experience. This context will be in a specific language (e.g., English or Spanish).'),
});
export type PortfolioChatbotInput = z.infer<typeof PortfolioChatbotInputSchema>;

const PortfolioChatbotOutputSchema = z.object({
  response: z.string().describe('The AI-generated response to the user query, in the same language as the portfolioContext.'),
});
...
const portfolioChatbotPrompt = ai.definePrompt({
  ...
  **Important: Respond in the same language as the provided 'portfolioContext'. For example, if the context is in Spanish, your entire response must be in Spanish.**
  Be friendly, professional, and concise. If you don't know the answer based on the provided context, politely state that you don't have that specific information (in the context's language).
  ...
  If the user requests a CV, resume, or curriculum, the CV content will be generated by another specialized flow. Your role in that case is simply to present the pre-generated Markdown content.

  Context about Brian Bentancourt (this context is in a specific language):
  {{{portfolioContext}}}

  User's question: {{{userQuery}}}

  Your response (in the same language as the portfolioContext):`,
   config: {
    safetySettings: [
      {
